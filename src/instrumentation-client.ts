...
if (dsn) {
  Sentry.init({
    ...
    // (`vigario-technology-solutions@${version}`). Matched values are how events tie
    // to source maps and release-context features in the dashboard.
    // Inlined into the client bundle at build time via next.config's
    // `env` block.
    ...
    // We're same-origin throughout (the marketing site doesn't talk
    // to a separate API host), so any relative URL gets propagation.
    // Made explicit so a future cross-origin split doesn't silently
    // lose traces.
    ...
    // false-positive that would otherwise burn through the free-tier
    // event quota without any actionable signal.
    ignoreErrors: [
      ...
      // User navigated away mid-fetch — `cancel()` surfaces as
      // AbortError; not actionable.
      "AbortError",
    ...
    integrations: [
      Sentry.replayIntegration({
        ...
        // safe to record (the contact / quote forms are PII-bearing).
        maskAllText: true,
        blockAllMedia: false,
      }),
    ],
