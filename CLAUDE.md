...
- Husky hooks: `pre-commit` runs `lint-staged` (with Windows defensive re-stage) → `typecheck` → `test`; `commit-msg` runs `commitlint`; `pre-push` enforces the `<type>/<slug>` branch-name convention, then `check:pins` → `lint:actions` → `build` → `check:bundles`. Local is a superset of the gate: everything CI checks is checked here first, except the dev-server smoke test, which needs a disposable machine.
